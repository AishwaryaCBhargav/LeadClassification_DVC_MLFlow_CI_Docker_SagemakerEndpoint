...
def main(config_path, params_path):
    ...
    prediction_directory_path = os.path.join(
        artifacts["artifacts_directory"], artifacts["prediction_directory"]
    )
    create_directories([prediction_directory_path])
    predicted_test_values = os.path.join(prediction_directory_path, artifacts["prediction_test"])
    predicted_test_csv = os.path.join(prediction_directory_path, artifacts["prediction_test_csv"])
    ...
    prediction_val = model.predict(scaler.transform(x_val))
    ...
    # np.save(predicted_test_values, prediction_test)
    # pd.DataFrame(prediction_test).to_csv(predicted_test_csv, header=True, index=False)

    # prediction_test.savetxt(predicted_test_values, header=False, index=False)
